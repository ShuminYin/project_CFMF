...
load("./output/section2/GEDs_of_celltype.RData")

save(list = c("seu","silhouette"),file = "./paper/result_section2/source_data/seu_of_pbmc3k.RData")
marker.df        <- silhouette %>% filter(-log10(FDR) > 200, silhouette > 0)
markers          <- marker.df %>% pull(gene)
marker.classical <- DEG %>% filter(p_val_adj < 0.01) %>% group_by(cluster) %>% arrange(desc(avg_log2FC)) %>% slice_head(n=10) %>% ungroup() %>% pull(gene)
others           <- setdiff(rownames(seu), markers)
length(marker.classical)
length(intersect(rownames(seu), marker.classical))

makers.in  <- length(intersect(markers, marker.classical))
makers.out <- length(markers) - makers.in
others.in  <- length(intersect(others, marker.classical))
others.out <- length(others) - others.in


df <- data.frame(Category = factor(rep(c("markers", "others"), each = 2), levels = c("markers", "others")),
                 Group = rep(c("In DEG", "Not in DEG"), 2),
                 Count = c(makers.in, makers.out, others.in, others.out))
df <- df %>% group_by(Category) %>% mutate(prop  = Count / sum(Count),
                                           label = percent(prop,accuracy = 0.01)) %>% ungroup()

contingency.mt <- matrix(c(makers.in, makers.out,others.in, others.out), nrow = 2,
                         dimnames = list(Group = c("markers", "others"),DEG = c("In DEG", "Not in DEG")))
fisher.result  <- fisher.test(contingency.mt)

save(list = c("df","contingency.mt", "fisher.result"), file = "./paper/result_section1/source_data/fisher_test_of_pbmc.RData")
...
silhouette |>
  ggplot(aes(y = -log10(FDR), x = silhouette)) +
  geom_point()
...
#######mian.2.F#######
load("./output/section2/seu and silhouette of Data_Brain_Neftel2019_SmartSeq2_TPM.RData")
genes.ls <- lapply(silhouette.ls, function(x) {
  genes <- x %>% filter(silhouette > 0, -log10(FDR) > 200) %>% pull(gene)
})
genes     <- unique(unlist(genes.ls))


###markers of GBM signature###
MPs       <- readxl::read_xlsx("./input/GBM/mmc2.xlsx", sheet = 1, skip = 4) |> as.list()
MPs.genes <- unique(unlist(MPs))


markers          <- unique(unlist(genes.ls))
GBM.sig          <- unique(unlist(MPs))
others           <- setdiff(rownames(seu.ma), markers)

makers.in  <- length(intersect(markers, GBM.sig))
makers.out <- length(markers) - makers.in
others.in  <- length(intersect(others, GBM.sig))
others.out <- length(others) - others.in


df <- data.frame(Category = factor(rep(c("markers", "others"), each = 2), levels = c("markers", "others")),
                 Group = rep(c("In GBM signature genesets", "Not in GBM signature genesets"), 2),
                 Count = c(makers.in, makers.out, others.in, others.out))
df <- df %>% group_by(Category) %>% mutate(prop  = Count / sum(Count),
                                           label = percent(prop, accuracy = 0.01)) %>% ungroup()

contingency.mt <- matrix(c(makers.in, makers.out,others.in, others.out), nrow = 2,
                         dimnames = list(Group = c("markers", "others"),DEG = c("In GBM signature genesets", "Not in GBM signature genesets")))

fisher.result  <- fisher.test(contingency.mt)

save(list = c("df","contingency.mt", "fisher.result"), file = "./paper/result_section1/source_data/fisher_test_of_GBM.RData")
...
#######mian.2.D#######
load("./output/section2/seu and silhouette of Data_Brain_Neftel2019_SmartSeq2_TPM.RData")
MPs           <- readxl::read_xlsx("./data/GBM/mmc2.xlsx", sheet = 1, skip = 4) |> as.list()
seu           <- AddModuleScore(seu, features = MPs, name = "GBM")
names         <- setNames(paste0("GBM", 1:length(MPs)), paste0("MP_", names(MPs)))
seu@meta.data <- seu[[]] |> dplyr::rename(all_of(names))
seu$MP_MES    <- (seu$MP_MES1 + seu$MP_MES2) / 2
seu$MP_NPC    <- (seu$MP_NPC1 + seu$MP_NPC2) / 2

save("seu",file = "./paper/result_section1/source_data/harmony_of_GBM.RData")
...
#######mian.2.C#######
load("./output/section2/seu and silhouette of Data_Brain_Neftel2019_SmartSeq2_TPM.RData")
df    <- silhouette.ls[[15]] |> mutate(y = -log10(FDR))
save("df",file = "./paper/result_section1/source_data/sil.df_of_GBM_sample.RData")
