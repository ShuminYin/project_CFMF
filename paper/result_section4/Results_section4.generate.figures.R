...
###############################mian.4.B########################################
load("./output/section4/CFMF markers harmony seu.malignant of tumor sample.RData")
...
save(list = c("seu","seu.malignant"), file = "./paper/result_section4/source_data/mian.4.B.RData")

###############################supp.3.A########################################
load("./output/section4/5. cancer subclone/harmony seu.malignant of tumor sample.RData")
...
save("seu.high", file = "./paper/result_section4/source_data/seu.high.RData")

#######################supp.7.A#################################################
load("./output/section4/5. cancer subclone/harmony seu.malignant of tumor sample.RData")
p1 <- FeaturePlot(seu, features = c("LGR5","TOP2A"), raster=FALSE) & NoLegend()

load("./output/section4/5. cancer subclone/CFMF markers harmony seu.malignant of tumor sample.RData")
...
#######################supp.4.B#################################################
load("./output/section4/subclone/CFMF markers harmony high quality seu.RData")
load("./output/section4/subclone/scoreMarkers of seu.high subclones.RData")

marker.2     <- foreach(i = markers.ls) %do% {gene <- head(i, 3)} %>% unlist()
marker.2[12] <- "LGR5"
...
save("marker.2", file = "./paper/result_section4/source_data/supp.4.B.RData")


#######################mian.4.D#################################################
load("./output/section4/subclone/hallmark.RData")
...
save(list = c("coldata","mat"), file = "./paper/result_section4/source_data/supp.4.B.RData")


#######################supp.3.B#################################################
load("./output/section4/subclone/reactiome.RData")

save(list = c("coldata","mat"), file = "./paper/result_section4/source_data/supp.3.B.RData")
...
#######################mian.4.F / supp.4.C#################################################
load("./out/5. cancer subclone/subclone/subclone.pro.df and immune.pro.df.RData")
save(list = c("subclone.pro.df", "immune.pro.df", "merged_df", "cor.results"), 
     file = "./paper/result_section4/source_data/mian.4.F.RData")
...
#######################mian.4.E#################################################
load("./output/section4/subclone/CFMF markers harmony high quality seu.RData")
load("./output/section4/subclone/Roe.RData")
library(ComplexHeatmap)
subclone.colors <- c(
  subclone1 = "#7FC8A9",
  subclone2 = "#8BB4D4",
...
save("Roe", file = "./paper/result_section4/source_data/mian.4.E.RData")

#######################mian.4.G#################################################
load("./output/section4/spatial/RCTD.plot.RData")
...
df <- foreach(i = names(rctd.ls), .combine = 'rbind') %do% {
  ## top 2 clusters with highest purity
  cl    <- purity.kmeans.cl.ls[[i]] |> group_by(kmeans) |> summarise(purity = median(purity, na.rm = TRUE)) |> arrange(desc(purity)) |> pull(kmeans) |> head(n = 2)
  spots <- purity.kmeans.cl.ls[[i]] |> na.omit() |> filter(kmeans %in% cl) |> pull(barcode)
  
  df    <- rctd.ls[[i]] |> filter(barcode %in% spots) |> mutate(sample = i, barcode = paste(i, barcode, sep = "_"), .before = everything())
}



mat     <- df |> column_to_rownames("barcode") |> select(starts_with("subclone")) |> as.matrix()

save("mat", file = "./paper/result_section4/source_data/mian.4.G.RData")
